...
export default async function AccountPage() {
  ...
  const [openConsumptions, settlementLines] = await Promise.all([
    ...
    prisma.settlementLine.findMany({
      ...
      include: {
        settlement: {
          select: { id: true, number: true, startDate: true, endDate: true, status: true, finalizedAt: true },
        },
      },
    ...
    <div className="space-y-6">
      <header className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-600">
          Review your open tab and past settlements. You can reverse mistakes before billing is finalized.
        </p>
      </header>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-600">Your last 12 billed periods.</p>
                ...
                <div className="mt-4 space-y-2">
                  {breakdown.map((item) => (
                    <div key={`${item.itemId}:${item.unitPriceCents}`} className="flex items-center justify-between text-sm">
                      <div className="text-slate-700">{item.itemName} × {item.quantity}</div>
